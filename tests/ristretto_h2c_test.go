...
func TestRistretto_HashToGroup(t *testing.T) {
	for i, test := range ristrettoH2gTests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			e := ristretto255.HashToGroup(v.input, v.dst)

			if !bytes.Equal(e.Encode(nil), v.encodedElement) {
				t.Fatalf(
					"Mappings do not match.\n\tExpected: %v\n\tActual: %v\n",
					hex.EncodeToString(v.encodedElement),
					hex.EncodeToString(e.Encode(nil)))
			}
		})
	}
}
...
func TestRistretto_EncodeToGroup(t *testing.T) {
	for i, test := range ristrettoH2gTests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			e := ristretto255.EncodeToGroup(v.input, v.dst)
